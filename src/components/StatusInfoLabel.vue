<template>
  <div :class="'status-label-container__' + type" class="status-label-container">
    ...
    <div v-else-if="state == 'completed' && results.length === 0" class="status-label-container__text">
      {{ $t( 'Unsuccessful') }}
    </div>
    ...
    <div :class="'status-label-container__sign_' + type " class="status-label-container__sign">
      <spinner-loader 
      v-if="state == 'pendingOfAuction' || state == 'pendingSyncData' || state == 'preAnnouncement' || remainedTimeOfRound < 0" />
      ...
      <svg v-else-if="state === 'completed' && results.length !== 0" class= "state_icon" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 22.12 17.01"><defs><clipPath id="d2e6e7f2-71df-4a16-823b-f86aae6e7c25" transform="translate(-4 -7)"><rect x="4" y="7" width="22.33" height="17.02" fill="none"/></clipPath></defs><g clip-path="url(#d2e6e7f2-71df-4a16-823b-f86aae6e7c25)"><path d="M4.25,16.36a.92.92,0,0,1-.25-.6.91.91,0,0,1,.25-.59L5.45,14a.82.82,0,0,1,1.19,0l.08.09,4.68,5a.41.41,0,0,0,.59,0L23.4,7.25h.09a.83.83,0,0,1,1.19,0l1.19,1.19a.82.82,0,0,1,0,1.19L12.25,23.76a.83.83,0,0,1-1.19,0L4.42,16.61Zm0,0" transform="translate(-4 -7)"/></g></svg>
      <svg v-else-if="state === 'completed' && results.length === 0 " class= "state_icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 19.07 19.07"><path d="M15.09,18.56" transform="translate(-6.84 -4.39)"/><path d="M14.07,18.54" transform="translate(-6.84 -4.39)"/><path fill="#000" d="M25.91,6.3,24,4.39,16.38,12,8.75,4.39,6.84,6.3l7.63,7.63L6.84,21.55l1.91,1.91,7.63-7.63L24,23.46l1.91-1.91-7.63-7.62Zm0,0" transform="translate(-6.84 -4.39)"/></svg>
